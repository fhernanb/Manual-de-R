# Creación de funciones en \proglang{R} \label{creafun}
...
knitr::include_graphics("images/midpoint")
```
...
11. Escriba una función llamada `minimo` que permita obtener el valor mínimo de un vector numérico. No puede usar ninguna de las funciones básicas de R como `which.min()`, `which.max()`, `order()`, `min( )`, `max( )`, `sort( )` u `order( )`. Use la estructura mostrada abajo.
