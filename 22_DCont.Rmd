...
En el espacio de las letras `xxx` se de debe colocar el nombre de la distribución en \proglang{R}, a continuación el listado de nombres disponibles para las 11 distribuciones continuas básicas.
...
Para calcular la probabilidad solicitada se usa nuevamente la función `pnorm` evaluada en el cuantil dado. Como el evento de interés es $Z > -0.37$, se obtener el complemento de la probabilidad obtenida con `pnorm(q=-0.37)`, esto debido a que por defecto las probabilidades entregadas siempre son a izquierda. A continuación el código usado.
...
Para calcular el cuantil en el cual se cumple que $P(Z<q)=0.95$ usa la función `qnorm`, a continuación el código usado.
...
Como se solicita probabilidad se debe usar `pnorm` indicando la media y la desviación de la distribución. A continuación el código usado.

```{r}
pnorm(q=11, mean=10, sd=2) - pnorm(q=9, mean=10, sd=2)
```

2) Un tornillo con un diámetro mayor a 11 mm se puede reprocesar. ¿Cuál es el porcentaje de reprocesos en la empresa?
...
Aquí interesa encontrar el cuantil tal que $P(Dia<q)=0.05$, por lo tanto se usa la función `qnorm`. A continuación el código usado.
...
Aquí interesa encontrar el cuantil tal que $P(Dia>q)=0.10$, por lo tanto se usa la función `qnorm` pero incluyendo `lower.tail=FALSE` por ser una cola a derecha. A continuación el código usado.
