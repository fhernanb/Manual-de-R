...
Al aplicar la prueba Shapiro Wilk a la muestra de alturas de los hombres se obtuvo un valor-P de `r round(shapiro.test(hombres$altura)$p.value, 4)`

Posterior a la descripción de la variable se usa la función `t.test` sobre la variable de interés: altura de los hombres. A continuación se muestra el código
 
```{r}
t.test(x=hombres$altura, conf.level=0.90)$conf.int
```

A partir del resultado obtenido se puede concluir, con un nivel de confianza del 90%, que la altura promedio de los estudiantes hombres se encuentra entre 176.438 cm y 181.7172 cm. 

### Intervalo de confianza bilateral para la diferencia de medias de muestras independientes 

Para calcular intervalos de confianza bilaterales para la diferencia de medias de muestras independientes a partir de la función `t.test` es necesario definir 5 argumentos: `x` considera la variable de interés en la muestra 1, `y` la variable de interés en la muestra 2, `paired = FALSE` indica que el intervalo de confianza se hará para muestras independientes, en el caso de que sean dependientes este argumento será `paired = TRUE`, `var.equal = FALSE` indica que las varianzas son desconocidas y diferentes y `conf.level` corresponde al nivel de confianza. 
...
```{r}
t.test(x = hombres$altura, y = mujeres$altura, paired=FALSE,   var.equal=FALSE, conf.level = 0.95)$conf.int
```
...
```{r}
t.test(x = rnorm(50, mean = 18, sd =3), alternative = "greater", conf.level = 0.90)$conf.int
```
