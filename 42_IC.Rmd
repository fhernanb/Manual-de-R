...
Para construir intervalos de confianza para la varianza se puede usar la función `var.test` del paquete básico **stats** o la función `var.test` del paquete **stests** [@R-stests] disponible en el [repositorio](https://github.com/fhernanb/stests). Ambas funciones tienen el mismo nombre la segunda es una generalización de la primera. En la siguiente nota se aclara la diferencia.
...
Una vez instalado el paquete **stests** se puede usar la función `var.test`, la cual es una generalización de la función `var.test` del paquete **stats** y por esa razón aparecerá el siguiente mensaje en la consola cuando se invoca el paquete.
...
```{block2, type='rmdnote'}
En este ejemplo no verificamos que la muestra aleatoria de alturas proviene de una población normal, eso ya lo habíamos hecho en el primer ejemplo de este capítulo.
...
```{r, eval=TRUE}
stests::var.test(x=hombres$altura, y=mujeres$altura,
         conf.level=0.95)$conf.int
```  
...
Los intervalos coinciden en los resultados.
...
```{block2, type='rmdwarning'}
¿Notó que las funciones `stats::var.test` y `stests::var.test` no son exactamente iguales?

`stats::var.test` solo sirve para construir IC para $\sigma_1^2 / \sigma_2^2$.

`var.test` sirve para construir IC para $\sigma^2$ y para $\sigma_1^2 / \sigma_2^2$.
```
