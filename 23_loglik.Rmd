...
```{r}
res <- optim(par=c(0, 0, 1), fn=minusll,
             method='L-BFGS-B',
             lower=c(-Inf, -Inf, 0),
             upper=c(Inf, Inf, Inf), y=y, x1=x1)
```

En el objeto `res` está el resultado de la optimización, para explorar los resultados usamos

```{r}
res
```

De la salida anterior se observa que el vector de parámetros estimado es `r res$par` y que el valor de la máxima log-verosimilitud fue de `r res$value`. Vemos entonces que el vector estimado está muy cerca del verdadero $\boldsymbol{\Theta}=(\beta_0, \beta_1, \sigma)^\top=(-2, 3, 5)^\top$.
