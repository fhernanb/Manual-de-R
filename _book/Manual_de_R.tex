...
de los hombres se obtuvo un valor-P de 0.3599, por lo tanto se asume que
la muestra de alturas provienen de una población normal.
...
es necesario definir 5 argumentos:

\begin{itemize}
\tightlist
\item
  \texttt{x} considera la variable de interés en la muestra 1,
\item
  \texttt{y} la variable de interés en la muestra 2,
\item
  \texttt{paired\ =\ FALSE} indica que el intervalo de confianza se hará
  para muestras independientes, en el caso de que sean dependientes este
  argumento será \texttt{paired\ =\ TRUE},
\item
  \texttt{var.equal\ =\ FALSE} indica que las varianzas son desconocidas
  y diferentes y
\item
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{t.test}\NormalTok{(}\DataTypeTok{x=}\NormalTok{hombres$altura, }\DataTypeTok{y=}\NormalTok{mujeres$altura,}
       \DataTypeTok{paired=}\OtherTok{FALSE}\NormalTok{, }\DataTypeTok{var.equal=}\OtherTok{FALSE}\NormalTok{,}
       \DataTypeTok{conf.level =} \FloatTok{0.95}\NormalTok{)$conf.int}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{t.test}\NormalTok{(}\DataTypeTok{x =} \KeywordTok{rnorm}\NormalTok{(}\DecValTok{50}\NormalTok{, }\DataTypeTok{mean =} \DecValTok{18}\NormalTok{, }\DataTypeTok{sd =}\DecValTok{3}\NormalTok{),}
       \DataTypeTok{alternative =} \StringTok{"greater"}\NormalTok{,}
       \DataTypeTok{conf.level =} \FloatTok{0.90}\NormalTok{)$conf.int}
\end{Highlighting}
\end{Shaded}
