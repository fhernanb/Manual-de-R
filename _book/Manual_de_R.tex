...
\subsection{¿Cómo extraer elementos de un
arreglo?}\label{como-extraer-elementos-de-un-arreglo-1}
...
\BeginKnitrBlock{rmdwarning}
La instalación del paquete con \texttt{install.packages} se hace sólo
una vez y no más. Cargar el paquete con \texttt{library} en la sesión se
debe hacer siempre que se vaya a usar el paquete.
\EndKnitrBlock{rmdwarning}
...
Para cada distribución discreta hay disponibles 4 funciones, a
continuación se muestra el listado de funciones y su utilidad.

\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{pxxx}\NormalTok{(q, ...)  }\CommentTok{# Función de masa de probabilidad, f(x)}
\KeywordTok{dxxx}\NormalTok{(x, ...)  }\CommentTok{# Función de distribución acumulada, F(x)}
...
En el espacio de las letras \texttt{xxx} se de debe colocar el nombre de
la distribución en \proglang{R}, a continuación el listado de nombres
disponibles para las 5 distribuciones discretas básicas.
...
\(f(x)\) de una binomial se usa la función \texttt{pbinom(\ )} y para
obtener la función acumulada \(F(x)\) de una Poisson se usa la función
\texttt{dpois(\ )}.

\subsection*{Ejemplo sobre distribución
binomial}\label{ejemplo-sobre-distribucion-binomial}


\index{binomial} Suponga que un grupo de agentes de tránsito sale a una
vía principal para revisar el estado de los buses de transporte
intermunicipal. De datos históricos se sabe que un 10\% de los buses
generan una mayor cantidad de humo de la permitida. En cada jornada los
agentes revisan siempre 18 buses, asuma que el estado de un bus es
independiente del estado de los otros buses.
...
\subsection*{Ejemplo sobre distribución
geométrica}\label{ejemplo-sobre-distribucion-geometrica}


\index{geométrica} En una línea de producción de bombillos se sabe que
sólo el 1\% de los bombillos son defectuosos. Una máquina automática
toma un bombillo y lo prueba, si el bombillo enciende, se siguen
probando los bombillos hasta que se encuentre \textbf{un} bombillo
defectuoso, ahí se para la línea de producción y se toman los
...
\subsection*{Ejemplo sobre distribución binomial
negativa}\label{ejemplo-sobre-distribucion-binomial-negativa}
\addcontentsline{toc}{subsection}{Ejemplo sobre distribución binomial
negativa}
...
\subsection*{Ejemplo sobre distribución
hipergeométrica}\label{ejemplo-sobre-distribucion-hipergeometrica}
\addcontentsline{toc}{subsection}{Ejemplo sobre distribución
hipergeométrica}
...
\subsection*{Ejemplo sobre distribución
Poisson}\label{ejemplo-sobre-distribucion-poisson}
...
\subsection*{Ejemplo: variable aleatoria discreta
general}\label{ejemplo-variable-aleatoria-discreta-general}
\addcontentsline{toc}{subsection}{Ejemplo: variable aleatoria discreta
general}
...
\section{\texorpdfstring{Funciones disponibles para distribuciones
continuas
\index{distribuciones continuas}}{Funciones disponibles para distribuciones continuas }}\label{funciones-disponibles-para-distribuciones-continuas}

Para cada distribución continua hay disponibles 4 funciones, a
continuación se muestra el listado de funciones y su utilidad.
...
probabilidad \(f(x)\) de una normal se usa la función \texttt{pnorm(\ )}
y para obtener la función acumulada \(F(x)\) de una Beta se usa la
función \texttt{dbeta(\ )}.

\subsection*{Ejemplo sobre distribución
beta}\label{ejemplo-sobre-distribucion-beta}


\index{beta} Considere que una variable aleatoria \(X\) se distribuye
beta con parámetros \(a=2\) y \(b=5\).
...
función \texttt{integrate}, los límites de la integral se ingresan en
por medio de los parámetros \texttt{lower} y \texttt{upper}. Si la
función a integrar tiene parámetros adicionales como en este caso, éstos
se ingresan luego de los límites de la integral. A continuación el
código necesario para obtener la probabiliad solicitada.
...
Otra forma de obtener la probabilidad solicitada es restando de la
probabilidad acumulada hasta el límite superior, la probabilidad
acumulada hasta el límite inferior. Las probabilidades acumuladas hasta
un valor dado se obtienen con la función \texttt{pbeta}, a continuación
el código necesario.
...
\subsection*{Ejemplo: variable aleatoria continua
general}\label{ejemplo-variable-aleatoria-continua-general}
\addcontentsline{toc}{subsection}{Ejemplo: variable aleatoria continua
general}
...
\texttt{plot}, a continuación el código necesario para dibujar la
densidad.
...
\caption{\label{fig:crabcont1}Función de densidad para el peso de los
cangrejos.}
\end{figure}
...
\begin{figure}[htbp]
\centering
\includegraphics{Manual_de_R_files/figure-latex/unnamed-chunk-200-1.pdf}
\caption{\label{fig:unnamed-chunk-200}Perfil de la función f(x).}
\end{figure}
