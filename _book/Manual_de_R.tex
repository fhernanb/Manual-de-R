...
\(Z > -0.37\), se obtener el complemento de la probabilidad obtenida con
\texttt{pnorm(q=-0.37)}, esto debido a que por defecto las
probabilidades entregadas siempre son a izquierda. A continuación el
código usado.
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{pnorm}\NormalTok{(}\DataTypeTok{q=}\NormalTok{-}\DecValTok{037}\NormalTok{, }\DataTypeTok{lower.tail=}\OtherTok{FALSE}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
Para calcular el cuantil en el cual se cumple que \(P(Z<q)=0.95\) usa la
función \texttt{qnorm}, a continuación el código usado.
...
Como se solicita probabilidad se debe usar \texttt{pnorm} indicando la
media y la desviación de la distribución. A continuación el código
usado.
...
\tightlist
\item
  Un tornillo con un diámetro mayor a 11 mm se puede reprocesar. ¿Cuál
  es el porcentaje de reprocesos en la empresa?
\end{enumerate}
...
Aquí interesa encontrar el cuantil tal que \(P(Dia<q)=0.05\), por lo
tanto se usa la función \texttt{qnorm}. A continuación el código usado.
...
Aquí interesa encontrar el cuantil tal que \(P(Dia>q)=0.10\), por lo
tanto se usa la función \texttt{qnorm} pero incluyendo
...
\begin{figure}[htbp]
\centering
\includegraphics{Manual_de_R_files/figure-latex/unnamed-chunk-212-1.pdf}
