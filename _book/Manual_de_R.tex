...
\section{¿En qué formato almacenar una base de
datos?}\label{en-que-formato-almacenar-una-base-de-datos}

Los archivos con la información para ser leídos por \proglang{R} se
pueden almacenar en formato plano con extensión .txt o en Excel con
extensión .csv.

En el Cuadro \ref{tab:dt1} se presenta una base de datos pequeña, tres
observaciones y tres variables que nos servirá como ejemplo para mostrar
...
embellecer el contenido. Por último se guarda el archivo en el lugar
deseado y al momento de guardarlo se debe modificar la opción tipo de
archivo a \textbf{csv (delimitado por comas)}.

\begin{figure}

{\centering \includegraphics[width=2.34in]{images/excel1} 
...
La función \texttt{read.table} se puede usar para leer o traer bases de
datos hacia \proglang{R}. La estructura de la función con los parámetros
más comunes de uso es la siguiente.
...
\item
  ...
  un url o una dirección del computador. Es posible usar
  \texttt{file.choose()} para que se abra un ventana y adjuntar el
  archivo deseado manualmente.
\item
  \texttt{header}: valor lógico, se usa \texttt{TRUE} si la primer línea
  de la base de datos tiene los nombres de las variables, caso contrario
  se usa \texttt{FALSE}.
\item
  \texttt{sep}: tipo de separación interna para los datos dentro del
  archivo. Usar \texttt{sep=\textquotesingle{};\textquotesingle{}}
  cuando el archivo tiene extension .csv, usar
  \texttt{sep=\textquotesingle{}\textquotesingle{}}, sin nada adentro,
  para .
\item
  ``: .
\end{itemize}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{url <-}\StringTok{ 'https://dl.dropboxusercontent.com/u/9602072/toy_data_set.csv'}
\NormalTok{datos <-}\StringTok{ }\KeywordTok{read.table}\NormalTok{(}\DataTypeTok{file=}\NormalTok{url, }\DataTypeTok{header=}\OtherTok{TRUE}\NormalTok{, }\DataTypeTok{sep=}\StringTok{';'}\NormalTok{)}
\NormalTok{datos}
\end{Highlighting}
\end{Shaded}
...
## 1 47.6  Femenino     1.57          29.5          13.9
## 2 68.1 Masculino     1.66          38.4          16.0
## 3 68.0 Masculino     1.90          36.5          16.6
## 4 80.0  Femenino     1.76          38.0          17.1
## 5 68.1 Masculino     1.83          38.0          17.1
## 6 56.1  Femenino     1.66          33.0          14.7
\end{verbatim}
...
\begin{figure}[htbp]
\centering
\includegraphics{Manual_de_R_files/figure-latex/unnamed-chunk-86-1.pdf}
\caption{\label{fig:unnamed-chunk-86}Perfil de la función f(x).}
\end{figure}
