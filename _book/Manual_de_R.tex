...
En este capítulo se mostrará cómo utilizar \proglang{R} para estudiar la
normalidad univariada de una muestra aleatoria.
...
con datos reales para mostrar la utilidad del histograma y gráfico de
...
\(n\)=10, 100, 1000 y 10000, para cada una de ellas construir los
gráficos de densidad.

A continuación se presenta el código necesario para construir la Figura
\ref{fig:normalidad1}. Se define el vector \texttt{n} con los valores
del tamaño de muestra y luego dentro de una sentencia \texttt{for} se
simula cada muestra y se dibuja su densidad.
...
De la Figura \ref{fig:normalidad1} se observa que, a pesar de haber
generado las muestra de una normal estándar, las densidades para las
muestras aleatorias no son perfectamente simétricas, sólo para el caso
de tamaño de muestra \(n=10000\) la densidad muestral fue bastante
simétrica. Esto significa que el gráfico de densidad se debe usar con
precaución para decidir sobre la normalidad de un conjunto de datos,
como regla práctica se aconseja lo siguiente:
...
\section{Gráficos cuantil cuantil}\label{graficos-cuantil-cuantil}
...
\href{https://www.youtube.com/watch?v=kx_o9rnI4DE}{enlace}. En la Figura
\ref{fig:normalidad3} se muestra un ejemplo de un QQplot y de sus
partes, los puntos y la línea de referencia. Si se tuviese una muestra
distribuída perfectamente normal, se esperaría que los puntos estuviesen
perfectamente alineados con la línea de referencia, sin embargo, las
muestran con las que se trabajan en la práctica casi nunca presentan
este comportamiento aún si fueron obtenidas de una población normal. En
la práctica se aceptan alejamientos del patrón lineal para aceptar que
los datos si provienen de una población normal.
...
A continuación se presentan tres ejemplos, dos con datos simulados y
...
A continuación se presenta el código necesario para construir la Figura
\ref{fig:normalidad4}. Se define el vector \texttt{n} con los valores
del tamaño de muestra y luego dentro de una sentencia \texttt{for} se
simula cada muestra \texttt{x} y se dibuja su QQplot.
...
cada muestra \texttt{x} de un \(N(0, 1)\), los puntos no se alinean de
forma perfecta, esto significa que en la práctica se debe ser prudente
con su uso, un alejamiento del patrón lineal NO significa que la muestra
no provenga de una población normal.
...
datos es por medio de las pruebas de normalidad. Las hipótesis para las
pruebas de normalidad son:
...
\item
  Prueba Shapiro-Wilk.
\item
  Prueba Anderson-Darling (del paquete \textbf{nortest}).
\item
  Prueba Cramer-von Mises (del paquete \textbf{nortest}).
\item
  Prueba Lilliefors (Kolmogorov-Smirnov) (del paquete \textbf{nortest}).
\item
  Prueba Pearson chi-square (del paquete \textbf{nortest}).
\item
  Prueba Shapiro-Francia (del paquete \textbf{nortest}).
\end{enumerate}
...
aplicar las pruebas Shapiro-Wilk y Anderson-Darling con un nivel de
significancia del 3\%.
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{require}\NormalTok{(nortest)  }\CommentTok{# Se debe instalar antes nortest}
...
\subsection*{Ejemplo QQplot para peso
corporal}\label{ejemplo-qqplot-para-peso-corporal-1}


Retomando la base de datos \textbf{medidas del cuerpo} presentada en el
Capítulo \ref{central}, se desea saber si el peso corporal, tanto de
hombres y mujeres, tiene una distribución normal usando pruebas de
normalidad, usar nivel de significancia 5\%.
...
Para aplicar la prueba Shapiro-Wilk se usa la función
\texttt{shapiro.test} a la lista \texttt{pesos} por medio de la función
\texttt{lapply}, a continuación el código usado.
