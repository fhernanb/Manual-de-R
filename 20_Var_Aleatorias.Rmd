...
Ahora vamos a verificar que es una función de densidad de probabilidad, es decir, que la integral es 1.
...
curve(expr=f, from=12.5, to=13,
      lwd=3, col="blue", las=1,
     xlab="X", ylab="f(X=x)")
```

La función $F(x)$ la podemos obtener integrando la función $f(x)$ de la siguiente manera:

$$
F(x) = \int_{12.5}^x f(t) dt = 20 e^{-20(t-12.5)} dt = 
$$
